...
/**
 * This class implements the Temporal operations generated into every type with a Temporal super
 * type.
 * 
 * @author jcmcote
 * 
...
public class TemporalUtil {
	...
	public static Date getTrimedNow() {
		return TemporalDateProvider.eINSTANCE.trimDate(TemporalDateProvider.eINSTANCE.now(),
				TemporalDateProvider.eINSTANCE.getGranularity());
	}

	/**
	 * If we are constructing an EObject with a Temporal super type we need to initialize its current
	 * date attribute and version holder.
	 */
	public static void initTemporal(EObject eObject) {
		if (eObject instanceof Temporal) {

			/**
			 * JCC: added this to support versioning of containment features. bypass temporality when
			 * setting up the verion holders.
			 */
	...
	/**
	 * This method returns the version corresponding to the given date. The version returned can
	 * either be the continuity or a versioned object. It returns the version that is earlier or equal
	 * to the given date. Null is returned if no version exists which is earlier than the given date.
	 */
	public static Temporal versionAt(EObject eObject, Date date) {
		...
		// Also note that versions are generally older than the continuity but could also be newer as in
		// the
		...
		if (TemporalDateProvider.eINSTANCE.areEqual(continuityDate, date,
				TemporalDateProvider.eINSTANCE.getGranularity())) {
			// Don't bother looking further we've got the best match.
			return continuity;
		}

		Date trimmedDate = TemporalDateProvider.eINSTANCE.trimDate(date,
				TemporalDateProvider.eINSTANCE.getGranularity());
	...
	public static boolean isDateWithinVersion(EObject eObject, Date date) {
		return TemporalDateProvider.eINSTANCE.areEqual(date, getDate(eObject),
				TemporalDateProvider.eINSTANCE.getGranularity());
	}
