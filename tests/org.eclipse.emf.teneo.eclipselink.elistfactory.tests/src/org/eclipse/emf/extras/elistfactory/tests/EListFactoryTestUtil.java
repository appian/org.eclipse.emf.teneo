...
public class EListFactoryTestUtil {
	...
	public static String getListConstructor(final EList<?> eList) {
		...
		if (eList instanceof FeatureMap) {
			...
			result += "<" + getEntryFeatureInstanceClassSimpleName(ecoreEMap, "key") + ","
					+ getEntryFeatureInstanceClassSimpleName(ecoreEMap, "value") + ">";
			result += "(";
	...
	private static String getEntryEClassLiteralName(final EcoreEMap<?, ?> ecoreEMap) {
		...
		result += ".Literals.";
		result += CodeGenUtil.format(ecoreEMap.getEStructuralFeature().getEType().getName(), '_', null, false, false)
				.toUpperCase();
		return result;
	}
	...
	private static String getSimpleEcoreEListTypeName(String qualifiedEcoreEListTypeName) {
		...
		String rawSimpleEcoreEListTypeName = qualifiedEcoreEListTypeName.substring(qualifiedEcoreEListTypeName
				.lastIndexOf(".") + 1, qualifiedEcoreEListTypeName.length());
	...
	public static GenClass findGenClass(final GenModel genModel, final EClass eClass) {
		...
		if (genPackage != null) {
			for (Object name : genPackage.getGenClasses()) {
				GenClass genClass = (GenClass) name;
	...
	public static GenFeature findGenFeature(final GenModel genModel, final EStructuralFeature eStructuralFeature) {
		...
		for (Object name : genClass.getGenFeatures()) {
			GenFeature genFeature = (GenFeature) name;
