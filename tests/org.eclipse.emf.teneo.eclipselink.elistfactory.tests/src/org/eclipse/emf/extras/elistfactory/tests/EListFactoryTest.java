...
/**
 * Retrieves all classes from a preset model, looks for all EList/EMap/FeatureMap features on theses classes and creates
 * the corresponding EList/EMap/FeatureMap instances for each of them. It concludes which constructors and which
 * arguments have been used therefore, and makes sure that they are the very same as those which would normally come
 * from the GenModel.
 */
public class EListFactoryTest extends TestCase {
	...
	// private static final String TEST_GENMODEL_URI =
	// "platform:/plugin/org.eclipse.emf.examples.library/model/extlibrary.genmodel";
	...
	public void testEListFactory() throws ClassNotFoundException {
		...
		for (Object name : ePackage.getEClassifiers()) {
			EClassifier eClassifier = (EClassifier) name;
			...
			if (eClassifier instanceof EClass) {
				...
				if (!eClass.isInterface() && !eClass.isAbstract()) {
					...
					for (Object name2 : eClass.getEAllStructuralFeatures()) {
						EStructuralFeature eStructuralFeature = (EStructuralFeature) name2;
						...
						if (!eStructuralFeature.isVolatile() && EElementUtil.isListType(eStructuralFeature)) {
							try {
								...
								String expectedEListConstructor = EListFactoryTestUtil.getListConstructorFromGenModel(
										genModel, eList);
								...
								assertEquals(
										"EList constructor as per BasicEListFactoryImpl does not match that coming from GenModel.",
										actualEListConstructor, expectedEListConstructor);
							} catch (Exception ex) {
								ex.printStackTrace();
							}
						}
