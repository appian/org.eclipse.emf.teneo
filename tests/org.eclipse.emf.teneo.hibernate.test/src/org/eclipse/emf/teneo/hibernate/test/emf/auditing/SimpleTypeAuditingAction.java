 ...
 */
public class SimpleTypeAuditingAction extends AbstractTestAction {
	...
	private static double DOUBLE2 = 10.85;
	private static float FLOAT2 = (float) 3258.95;
	private static boolean BOOL2 = false;
	private static byte BYTE2 = (byte) 220;
	private static int INT2 = 5430;
	private static short SHORT2 = (short) 426;
	private static long LONG2 = 278923;
	private static Date DATE2 = new Date(System.currentTimeMillis() + 1000);
	private static String STRING2 = "01231231";
	private static BigDecimal bigDecimalOne2 = new BigDecimal("11.456778");
	private static BigDecimal bigDecimalTwo2 = new BigDecimal("11.4");
	...
	public void doAction(TestStore store) {
		...
		{
			store.beginTransaction();
			SimpleType stype = store.getObject(SimpleType.class);
			stype.setCode("This is a test simpletype2");
			stype.setBoo(BOOL2);
			stype.setByt(BYTE2);
			stype.setDat(DATE2);
			stype.setDoubl(DOUBLE2);
			stype.setEnu(SimpleEnum.ENUM2_LITERAL);
			stype.setFloat(FLOAT2);
			stype.setInte(INT2);
			stype.setLimitedstring(STRING2);
			stype.setLon(LONG2);
			stype.setShor(SHORT2);
			stype.setLimitedDecimal(bigDecimalOne2);
			store.store(stype);
			store.commitTransaction();
		}
		...
		{
			store.beginTransaction();
			final HbDataStore dataStore = ((HibernateTestStore) store).getEmfDataStore();
			final AuditVersionProvider vp = dataStore.getAuditVersionProvider();
			SimpleType stype = store.getObject(SimpleType.class);
			final List<TeneoAuditEntry> auditEntries = vp.getAllAuditEntries(stype);
			assertTrue(auditEntries.size() == 2);
			{
				final SimpleType result = (SimpleType) vp.getRevision(auditEntries.get(0));
			...
			{
				final SimpleType result = (SimpleType) vp.getRevision(auditEntries.get(1));
				assertEquals(BOOL2, result.isBoo());
				assertEquals(BYTE2, result.getByt());
				assertTrue(equalDates(store, result.getDat(), DATE2));
				assertEquals(DOUBLE2, result.getDoubl(), 0.01);
				assertTrue(result.getEnu().equals(SimpleEnum.ENUM2_LITERAL));
				assertEquals(FLOAT2, result.getFloat(), 0.01);
				assertEquals(INT2, result.getInte());
				assertTrue(result.getLimitedstring().compareTo(STRING2) == 0);
				assertEquals(LONG2, result.getLon());
				assertEquals(SHORT2, result.getShor());
				// hsqldb does not support column length, at least not in in-mem
				// mode
				if (!(store.getDatabaseAdapter() instanceof HsqldbTestDatabaseAdapter)) {
					assertEquals(STRING2.substring(0, 5), result.getExtraLimitedString());
				}
				assertEquals(bigDecimalTwo2.floatValue(), result.getLimitedDecimal().floatValue(), 0.11);
			}
			store.commitTransaction();
		}
...

		{
			...
			stype.setCode("This is a test simplelisttype2");
			store.store(stype);
			store.commitTransaction();
		}
		...
		}
		{
			final HbDataStore dataStore = ((HibernateTestStore) store).getEmfDataStore();
			final AuditVersionProvider vp = dataStore.getAuditVersionProvider();
			SimpleList stype = store.getObject(SimpleList.class);
			final List<TeneoAuditEntry> auditEntries = vp.getAllAuditEntries(stype);
			assertTrue(auditEntries.size() == 3);
			{
				SimpleList result = (SimpleList) vp.getRevision(auditEntries.get(0));
				assertValues(result.getBoo(), bools);
				assertValues(result.getByt(), bytes);
				assertValues(result.getShor(), shorts);
				assertValues(result.getInte(), ints);
			...
			{
				SimpleList result = (SimpleList) vp.getRevision(auditEntries.get(1));
				assertValues(result.getBoo(), bools);
				assertValues(result.getByt(), bytes);
				assertValues(result.getShor(), shorts);
				assertValues(result.getInte(), ints);
			...
			assertTrue(vp.getRevision(auditEntries.get(1)) != vp.getRevision(auditEntries.get(0)));
		}
	}
