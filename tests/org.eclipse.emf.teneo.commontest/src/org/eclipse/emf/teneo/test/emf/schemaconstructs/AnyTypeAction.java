 ...
 * </copyright>
 *
 * $Id: AnyTypeAction.java,v 1.4 2007/02/01 12:35:37 mtaal Exp $
 */
	...
	public void doAction(TestStore store)
	{
    	...
    	}
    	{
    		...
    		for (int i = 0; i < testAny.getMultiAnyType().size(); i++)
    		{
    			...
    			{
    				assertTrue("The multiAnyType should have only 2 elements but it has " + testAny.getMultiAnyType().size(), false);
    			}
    		}
    		
    		// test xsd:any
    		assertTrue("Size of testAny.getAny should be 3 but it is: " + testAny.getAny().size(), testAny.getAny().size() == 3);
    		for (int i = 0; i < testAny.getAny().size(); i++)
    		{
    			final FeatureMap.Entry entry = (FeatureMap.Entry)testAny.getAny().get(i);
    			if (i == 0)
    			{
    			...
    			else if (i == 1)
    			{
    				assertTrue("Feature expected/found: " + AnytypePackage.eINSTANCE.getA_Doub().getName() + 
    						"/" + entry.getEStructuralFeature().getName(), AnytypePackage.eINSTANCE.getA_Doub() == entry.getEStructuralFeature());
    			...
    			else if (i == 2)
    			{
    				assertTrue("Feature expected/found: " + AnytypePackage.eINSTANCE.getA_MyB().getName() + 
    						"/" + entry.getEStructuralFeature().getName(), AnytypePackage.eINSTANCE.getA_MyB() == entry.getEStructuralFeature());
	        ...
	        testAny.getMyAny().add(AnytypePackage.eINSTANCE.getA_MyB(), b5);
    		*/
    		assertTrue("Size of testAny.getMyAny should be 3 but it is: " + testAny.getMyAny().size(), testAny.getMyAny().size() == 3);
    		for (int i = 0; i < testAny.getMyAny().size(); i++)
    		{
    			final FeatureMap.Entry entry = (FeatureMap.Entry)testAny.getMyAny().get(i);
    			if (i == 0)
    			{
    				assertTrue("Feature expected/found: " + AnytypePackage.eINSTANCE.getB_Name().getName() + 
    						"/" + entry.getEStructuralFeature().getName(), AnytypePackage.eINSTANCE.getB_Name() == entry.getEStructuralFeature());
