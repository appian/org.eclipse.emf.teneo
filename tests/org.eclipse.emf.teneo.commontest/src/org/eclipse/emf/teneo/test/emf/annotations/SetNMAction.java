 ...
 * </copyright>
 *
 * $Id: SetNMAction.java,v 1.1 2006/07/04 22:12:14 mtaal Exp $
 */
...
public class SetNMAction extends AbstractTestAction 
{
	...
	public void doAction(TestStore store)
	{
    	...
    	{
    		store.beginTransaction();
    		List list = store.getObjects(MyItem.class);
...

    	{
    		...
    		for (int i = 0; i < list.size(); i++)
    		{
    			...
    			if (i < (list.size() - 1))
    			{
    				assertEquals("yourname_" + (i + 1), ((YourItem)((MyItem)list.get(i)).getYourItem().get(i)).getName());
    			}
    		}
	        store.commitTransaction();
    	}
