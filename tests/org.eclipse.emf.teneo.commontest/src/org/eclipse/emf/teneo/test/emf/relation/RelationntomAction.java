 ...
 * </copyright>
 *
 * $Id: RelationntomAction.java,v 1.1 2006/07/04 22:12:17 mtaal Exp $
 */
...
public class RelationntomAction extends AbstractTestAction 
{
	...
	public void doAction(TestStore store)
	{
...

    	{
    		...
    		final ArrayList multinns = new ArrayList();    		
    		...
    		final ArrayList multinrs = new ArrayList();
       		for (int i = 0; i < NO_MULTIS; i++)
    		{
    		...
    		final ArrayList multirns = new ArrayList();
       		for (int i = 0; i < NO_MULTIS; i++)
    		{
...

       	{
    		...
    		for (int i = 0; i < mains.size(); i++)
    		{
    			...
    			{
    				MultiNN multinn = (MultiNN)main.getMultinn().get(k);
    			...
    			{
    				MultiNR multinr = (MultiNR)main.getMultinr().get(k);
    			...
    			{
    				MultiRN multirn = (MultiRN)main.getMultirn().get(k);
        			assertEquals(NO_MAINS, multirn.getMain().size());    				
        			assertTrue(mains.containsAll(multirn.getMain()));
    			}
    		}
    		
			//prod = (ProductType)store.query("SELECT FROM org.eclipse.emf.teneo.test.emf.catalog.gen.impl.ProductTypeImpl WHERE remark.contains(rem) && rem.value == \"remarka\" VARIABLES org.eclipse.emf.teneo.test.emf.catalog.gen.impl.StringTypeImpl rem", 1).get(0);
    		store.commitTransaction();
    	}
    	...
    	{	
    		...
    		// restore again
       		store.beginTransaction();
       	 	...
       	 	assertTrue("Size of all collections is 2", main.getOnecr().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getOnecn().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getOnenr().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getOnenn().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getTwocr().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getTwocn().size() == 100);
       	 	assertTrue("Size of all collections is 2", main.getTwonr().size() == 2);
       	 	assertTrue("Size of all collections is 2", main.getTwonn().size() == 100);
       	 	
       	 	assertTrue("EContainer is null", ((EObject)main.getOnecn().get(0)).eContainer() != null);
       	 	assertTrue("EContainer is null", ((EObject)main.getOnecr().get(1)).eContainer() != null);
    	...
    	// and do some counts
    	{
    		...
    		store.checkNumber(TwoCN.class, 100); //0
    		store.checkNumber(TwoNR.class, 2);
    		store.checkNumber(TwoNN.class, 100);
    		store.commitTransaction();
