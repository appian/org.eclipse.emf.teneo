 ...
 * </copyright>
 *
 * $Id: CatalogAction.java,v 1.2 2007/02/01 12:35:37 mtaal Exp $
 */
...
public abstract class CatalogAction extends AbstractTestAction 
{
	...
	public void doAction(TestStore store)
	{
    	...
    	// now retrieve the second product and check it
    	{
			...
			final StringType remark = (StringType)result.getRemark().get(0); 
			assertTrue(remark.eContainer() != null);
			
			assertTrue(((StringType)result.getRemark().get(0)).getValue().compareTo("remarkb") == 0);
    	...
    	// select a product based on one of its remark values
       	{
	        ...
	        assertTrue(((StringType)prod.getRemark().get(0)).getValue().compareTo("remarka") == 0);
    		store.commitTransaction();
    	}
	}
