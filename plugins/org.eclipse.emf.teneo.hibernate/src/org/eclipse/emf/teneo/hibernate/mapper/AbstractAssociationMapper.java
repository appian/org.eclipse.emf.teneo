...
abstract class AbstractAssociationMapper extends AbstractMapper {
	...
	protected Element addManyToOne(String assocName, String assocEntity) {
		log.debug("addManyToOne " + assocName + "/" + assocEntity);

		Element associationElement = getHbmContext().getCurrent().addElement("many-to-one").addAttribute("name",
				assocName).addAttribute("entity-name", assocEntity);
