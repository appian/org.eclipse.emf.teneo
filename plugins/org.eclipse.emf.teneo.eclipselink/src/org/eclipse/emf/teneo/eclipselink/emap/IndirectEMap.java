...
public class IndirectEMap<K, V> extends BasicEMap<K, V> implements IndirectEContainer<Map.Entry<K, V>>, InternalEList.Unsettable<Map.Entry<K, V>>, EStructuralFeature.Setting {
  ...
  /**
   * PUBLIC: Construct a new, empty EmfIndirectEMap with a default capacity and
   * load factor.
   */
  public IndirectEMap() {
  ...
  /**
   * PUBLIC: Construct a new, empty EmfIndirectEMap with the specified initial
   ...
   *              if the initial capacity is less than or equal to zero, or if
  ...
  /**
   * PUBLIC: Construct a new EmfIndirectEMap with the same mappings as the given
   * Map. The EmfIndirectEMap is created with a capacity of twice the number of
   * entries in the given Map or 11 (whichever is greater), and a default load
  ...
  /**
   * Used during postBuild/Merge/Clone to replace generic EclipseLinkEMap with the
   * correct EMF collection class
   * 
   * @param eMap
   */
  ...
  /**
   * @see java.util.Hashtable#clone() This will result in a database query if
   *      necessary.
   */

  /*
   * There are 3 situations when clone() is called: 1. The developer actually
   * wants to clone the collection (typically to modify one of the 2 resulting
   * collections). In which case the contents must be read from the database. 2.
   * A UnitOfWork needs a clone (or backup clone) of the collection. But the
   * UnitOfWork checks "instantiation" before cloning collections
   * ("un-instantiated" collections are not cloned). 3. A MergeManager needs an
   * extra copy of the collection (because the "backup" and "target" are the
   * same object?). But the MergeManager checks "instantiation" before merging
   * collections (again, "un-instantiated" collections are not merged).
   */
  @SuppressWarnings("unchecked")
  @Override
  ...
  /**
   * Check whether the contents have been read from the database. If they have
   * not, read them and set the delegate.
   */
  ...
  /**
   * Return if the collection has a property change listener for change
   * tracking.
   */
      ...
      public Iterator<K> iterator() {
        ...
        return new Iterator<K>() {
          ...
          public void remove() {
            ...
            IndirectEMap.this.raiseRemoveChangeEvent(currentKey, IndirectEMap.this.getDelegate().get(currentKey));
            this.delegateIterator.remove();
          }
        };
      }
  ...
  /**
   * Set the mapping attribute name, used to raise change events. This is
   * required if the change listener is set.
   */
  ...
  /**
   * PUBLIC: Use the Hashtable.toString(); but wrap it with braces to indicate
   * there is a bit of indirection. Don't allow this method to trigger a
   * database read.
   * 
   ...
   * (non-Javadoc)
   * 
   * @see org.eclipse.emf.common.util.BasicEMap#addAll(int,
