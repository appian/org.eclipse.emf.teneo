...
import org.eclipse.emf.teneo.eclipselink.common.eobject.InternalDatabaseEObject;

/**
 * This class contains convenient static methods for working with EclipseLink
 * resources.
 */
public class EclipseLinkResourceUtil {

  /**
   * Creates a EclipseLink {@link org.eclipse.emf.common.util.URI URI} from the
   * given EclipseLink settings.
   * <p>
   * The resulting EclipseLink {@link org.eclipse.emf.common.util.URI URI} will have
   ...
   *          using
   *          {@link org.eclipse.emf.teneo.eclipselink.common.resource.EclipseLinkResource.OPTION_SESSIONS_CONFIGURATION_FILE_NAME OPTION_SESSIONS_CONFIGURATION_FILE_NAME}.
   ...
   * @return the newly created EclipseLink
   *         {@link org.eclipse.emf.common.util.URI URI} instance.
   */
  ...
  /**
   * Checks if given {@link org.eclipse.emf.common.util.URI URI} is a EclipseLink
   * URI.
   * 
   * @param uri
   *          the {@link org.eclipse.emf.common.util.URI URI} to be analyzed.
   *          Must not be null.
  ...
  /**
   * Creates a {@link org.eclipse.persistence.queries.ReadAllQuery contents query}
   * for a given type with a given value of a given feature.
   * 
   * @param contentsType
   *          the type which the contents query has to be created for. Must not
   *          be null.
   * @param feature
   *          the feature on given type which is relevant to the contents query.
   *          Must not be null.
   * @param value
   *          the value which contents query to be created will check for
   *          equality on given feature of objects with given type. Must not be
   *          null.
   * @return the newly created
   *         {@link org.eclipse.persistence.queries.ReadAllQuery contents query}
   */
  public static String createContentsEqualQuery(EClass contentsType, EStructuralFeature feature, String value) {
    ...
    if (value == null || value.trim().length() == 0) {
      String msg = "Argument for parameter 'value' must not be blank.";
      throw new WrappedException(new IllegalArgumentException(msg));
    }
  ...
  public static String createContentsExampleQuery(EObject contentsExample) {
    ...
    String result = "";
    if (contentsExample instanceof InternalDatabaseEObject) {
      InternalDatabaseEObject internalContentsExample = (InternalDatabaseEObject) contentsExample;
      result = "select o from " + contentsExample.eClass().getName() + " o where o.dbID = '" + internalContentsExample.getDbID() + "'";
    }
    return result;
  }
}
