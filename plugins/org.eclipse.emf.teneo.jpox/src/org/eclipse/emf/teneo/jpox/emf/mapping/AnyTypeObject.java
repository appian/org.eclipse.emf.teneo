 ...
 * </copyright>
 *
 * $Id: AnyTypeObject.java,v 1.1 2006/07/08 22:04:29 mtaal Exp $
 */
		...
		// a general object
			...
			if (classifier.compareTo(Date.class.getName()) == 0) {
				try {
					...
					throw new JpoxStoreException("Can not create date object from: " + value, e);
				}
			} else {
				try {
					...
					throw new JpoxStoreException("The class: " + classifier
							+ " could not be found, value in the db is " + value, e);
				} catch (NoSuchMethodException e) {
					throw new JpoxStoreException("The class " + classifier
							+ " does not have a constructor which expects one String parameter", e);
				} catch (Exception e) {
					throw new JpoxStoreException("Exception when translation " + classifier + "/" + value
							+ " to an Object", e);
				}
			}
		}
