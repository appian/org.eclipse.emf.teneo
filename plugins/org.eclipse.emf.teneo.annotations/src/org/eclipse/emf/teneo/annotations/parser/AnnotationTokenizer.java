 ...
 * </copyright>
 *
 * $Id: AnnotationTokenizer.java,v 1.9.2.1 2007/02/11 20:44:01 mtaal Exp $
 */
	...
	final int getNextToken() {
		...
		Loop: for (;;) {
			...
			switch (lChar) {
				...
				{
					if (lCur == length) // Guard against embedded nulls in the Source.
					{
				...
				case '"': {
					...
					if (lChar != '\"') {
						final AnnotationParserException e = new AnnotationParserException("Value not closed with double quote, see the _ for the location " 
								+ getErrorText());
