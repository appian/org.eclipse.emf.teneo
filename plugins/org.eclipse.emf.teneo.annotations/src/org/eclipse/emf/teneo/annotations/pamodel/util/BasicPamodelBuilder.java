...
public class BasicPamodelBuilder {
	...
	/**
	 * @return a <code>PAnnotatedModelElement</code> assoiciated to the
	 * given <code>EModelElement</code>. The element is created only if
	...
	/**
	 * @return a <code>PAnnotatedEPackage</code> associated to the
	 * given <code>EPackage</code> and adds it the model.
	 * <p> The <code>PAnnotatedEPackage</code> is created only if 
	 * not already present in the model. 
	 */
	protected PAnnotatedEPackage pElement(EPackage ePackage) {
		PAnnotatedEPackage pPackage = (PAnnotatedEPackage) create(ePackage);
		if (pPackage.eContainer() == null) // means it is a fresh object
			target.getPaEPackages().add(pPackage);
		return pPackage;
	}
	
	/**
	 * @return a <code>PAnnotatedEClass</code> associated to the
	 * given <code>EClass</code> and adds it the model.
	 * <p> The <code>PAnnotatedEClass</code> is created only if 
	 * not already present in the model.
	 ...
	 * <code>EPackage</code> (if such an <code>PAnnotatedEPackage</code> 
	 * does not yet exists) and adds it to the target model.
	 */
	 ...
	 * <code>EClass</code> (if such an <code>PAnnotatedEClass</code> 
	 * does not yet exists) and adds it to the target model.
	 * 
	 * <p>The creation of a new <code>PAnnotatedEClass</code> may
	 * involve the creation of a <code>PAnnotatedEPackage</code>
	 * associated to the containing <code>EPackage</code> of the
	 * given class.
	 */
	...
	/**
	 * Add to the the target model a new <code>PAnnotatedEStructuralFeature</code>
	 * refering to the given EStructuralFeature.
	 * 
	 * <p>A PAnnotatedEClass and a PAnnotatedEPackage for the containing
	 * EClass and EPackage are added if needed.
	 * 
	 * <p> The added element have no annotations. Elements for which
	 * a corresponding PAnnotatedElement is already present in the
	 * target model are ignored.  
	 */
	 ...
	 * 
	 * <p>This operation may involve the addition to the model of a newly
	 * created PAnnotatedEModelElement for the PAnnotation EModelElement.
	 * @throws NullPointerException if either <code>pAnnotation</code> or
	 ...
	 * to the given EPackage. Recursively adds a PAnnotatedEClass for
	 * each EClass in the given EPackage (see {@link addEClass}).
	 * 
	 * <p> The added elements have no annotations. Elements for which
	 * a corresponding PAnnotatedElement is already present in the
	 * target model are ignored.  
	 */
	...
	/**
	 * used by {@link #addRecurse(EPackage)} to avoid recomputing the
	...
	/**
	 * Add to the the target model a new PAnnotatedPackage refering
	 * to the given EClass. Recursively adds a PAnnotatedEStructuralFeature
	 * for each EStructuralFeature in the given EClass
	 * (see {@link addEStructuralFeature}).
	 * 
	 * <p>A PAnnotatedEPackage for the containng EPackage is added if needed.
	 * 
	 * <p> The added elements have no annotations.
	 * 
	...
	protected void add(PAnnotatedEClass paClass, EStructuralFeature eFeature) {
		...
		if (paFeature.eContainer() == null) // meaning it is fresh
