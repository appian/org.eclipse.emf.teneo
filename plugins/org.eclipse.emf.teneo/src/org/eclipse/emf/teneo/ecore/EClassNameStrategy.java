...
/**
 * Resolves human usable unique names for EClasses. Internally Elver always uses
 * qualified String identifiers (EPackage nsuri concatenated with the EClass
 * name). However often the EClass name is unique and much easier to use for
 * humans.
 * 
...
public interface EClassNameStrategy {
	 ...
	 * EOBJECT_ECLASS_URI must be returned.
	 * 
	 * @param eClass
	 ...
	 * 
	 * Note if the name corresponds to EOBJECT_ECLASS_URI then the EcorePackage
	 * EObject must be returned.
	 * 
	 * @return eClass the resolved EClass
	 * @param The
	 *            EClassName
	 * @throws IllegalArgumentException
	 *             if the EClass can not be resolved.
