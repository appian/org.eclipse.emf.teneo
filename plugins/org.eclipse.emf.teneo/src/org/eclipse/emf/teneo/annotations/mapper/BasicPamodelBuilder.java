 ...
 * </copyright>
 *
 * $Id: BasicPamodelBuilder.java,v 1.5 2009/03/30 06:40:59 mtaal Exp $
 */
...
public class BasicPamodelBuilder implements ExtensionPoint {
	 ...
	 *         <code>EModelElement</code>. The element is created only if not
	 ...
	 *         to alter how objects are created. If you'd like to alter any
	 *         other logic around the creation, you should override the
	 *         <code>create</code> method(s).
	 * @throws AssertionError
	 */
	protected PAnnotatedEModelElement doCreate(EModelElement eModelElement)
			throws AssertionError {
		...
		default:
			throw new AssertionError(
					"Trying to build PAnnotatedEModelElement for a "
							+ eModelElementEClass);
		}
	...
	/**
	 * @return a <code>PAnnotatedEPackage</code> associated to the given
	 *         <code>EPackage</code> and adds it the model.
	 *         <p>
	 *         The <code>PAnnotatedEPackage</code> is created only if not
	 *         already present in the model.
	 */
	...
	/**
	 * @return a <code>PAnnotatedEClass</code> associated to the given
	 *         <code>EClass</code> and adds it the model.
	 *         <p>
	 *         The <code>PAnnotatedEClass</code> is created only if not already
	 *         present in the model.
	 *         <p>
	 *         The operation may involve the creation of a
	 *         <code>PAnnotatedEPackage</code> associated to the given
	 ...
	 *         <p>
	 *         The <code>PAnnotatedEStructuralFeature</code> is created only if
	 *         not already present in the model.
	 *         <p>
	 *         The operation may involve the creation of a
	 *         <code>PAnnotatedEPackage</code> and a
	...
	protected PAnnotatedEModelElement pElement(EStructuralFeature eFeature) {
		...
		pElement(eFeature.getEContainingClass()).getPaEStructuralFeatures()
				.add(pFeature);
		return pFeature;
	}
	 ...
	 */
	protected PAnnotatedEModelElement pElement(final EModelElement eElement)
			throws AssertionError {
	...
	/**
	 * Builds a <code>PAnnotatedEPackage</code> associated to the given
	 * <code>EPackage</code> (if such an <code>PAnnotatedEPackage</code> does
	 * not yet exists) and adds it to the target model.
	 */
	...
	/**
	 * Builds a <code>PAnnotatedEClass</code> associated to the given
	 * <code>EClass</code> (if such an <code>PAnnotatedEClass</code> does not
	 * yet exists) and adds it to the target model.
	 * 
	 * <p>
	 * The creation of a new <code>PAnnotatedEClass</code> may involve the
	 * creation of a <code>PAnnotatedEPackage</code> associated to the
	 * containing <code>EPackage</code> of the given class.
	 */
	...
	/**
	 * Add to the the target model a new
	 * <code>PAnnotatedEStructuralFeature</code> refering to the given
	 * EStructuralFeature.
	 * 
	 * <p>
	 * A PAnnotatedEClass and a PAnnotatedEPackage for the containing EClass and
	 * EPackage are added if needed.
	 * 
	 * <p>
	 * The added element have no annotations. Elements for which a corresponding
	 * PAnnotatedElement is already present in the target model are ignored.
	 */
	 ...
	 *             if the given PAnnotation is not admitted for the given
	 *             PAnnotatedEModelElement. protected void
	 *             setPAnnotation(PAnnotatedEModelElement pElement, PAnnotation
	 *             pAnnotation) { EReference pAnnotationRef =
	 *             PamodelPackage.eINSTANCE
	 *             .pAnnotationReference(pElement.eClass(),
	 *             pAnnotation.eClass()); if (pAnnotationRef == null) throw new
	 *             IllegalArgumentException("PAnnotation of type '" +
	 *             pAnnotation.eClass() + "' does not apply to elements of type
	 *             '" + pElement.eClass() + "'"); pElement.eSet(pAnnotationRef,
	 *             pAnnotation); }
	 */
	 ...
	 *             <code>PAnnotatedElement</code> public void add(PAnnotation
	 *             pAnnotation) { PAnnotatedEModelElement pElement =
	 *             pElement(pAnnotation.getEModelElement());
	 *             setPAnnotation(pElement, pAnnotation); }
	 */

	/**
	 * Add to the the target model a new PAnnotatedPackage refering to the given
	 * EPackage. Recursively adds a PAnnotatedEClass for each EClass in the
	 * given EPackage (see {@link addEClass}).
	 * 
	 * <p>
	 * The added elements have no annotations. Elements for which a
	 * corresponding PAnnotatedElement is already present in the target model
	 * are ignored.
	 */
	...
	/**
	 * used by {@link #addRecurse(EPackage)} to avoid recomputing the container
	 * multiple times.
	 */
	...
	/**
	 * Add to the the target model a new PAnnotatedPackage refering to the given
	 * EClass. Recursively adds a PAnnotatedEStructuralFeature for each
	 * EStructuralFeature in the given EClass (see {@link addEStructuralFeature}
	 * ).
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * Elements for which a corresponding PAnnotatedElement is already present
	 * in the target model are ignored. public void addRecurse(EClass eClass) {
	 * addRecurse((PAnnotatedEPackage) pElement(eClass), eClass); }
	 */

	/**
	 * used by {@link #addRecurse(EClass)} to avoid recomputing the container
	 * multiple times.
	 */
