 ...
 * </copyright>
 *
 * $Id: EFeatureAnnotator.java,v 1.11 2009/03/30 06:40:59 mtaal Exp $
 */
...
/**
 * Sets the annotation on an efeature. In fact determines which efeature
 * annotator to use (one-to-many, many-to-many etc.).
 * 
 * @author <a href="mailto:mtaal@elver.org">Martin Taal</a>
 * @version $Revision: 1.11 $
 */

public class EFeatureAnnotator extends AbstractAnnotator implements
		ExtensionPoint {
	...
	public void annotate(PAnnotatedEStructuralFeature aStructuralFeature) {
		...
		try {
			...
			if (aStructuralFeature instanceof PAnnotatedEAttribute) {
				...
				// place a mto
				final boolean otoBidirectionalRelation = aReference
						.getManyToOne() == null
						&& !isMany && eOpposite != null && !isOppositeMany;
				final boolean otoUnidirectionalRelation = aReference
						.getManyToOne() == null
						&& !isMany
						&& eOpposite == null
				...
				if (mtmBidirectionalRelation) {
					...
					oneToManyReferenceAnnotator.annotate(aReference);
						...
						&& (otoBidirectionalRelation || otoUnidirectionalRelation)) {
