 ...
 * </copyright>
 *
 * $Id: FieldUtil.java,v 1.8 2007/02/01 12:34:21 mtaal Exp $
 */
...
public class FieldUtil {
	...
	private static final Hashtable fieldMethodCache = new Hashtable();
	...
	public static Object callMethod(Object obj, String methodName, Object[] params) {
		...
		try {
			...
			if (method != null) {
				...
				throw new StoreException("Method does not exist " + obj.getClass().getName() + " method; "
						+ method.getName());
			}
			
			return method.invoke(obj, params);
		} catch (Exception e) {
			throw new StoreException("Exception " + obj.getClass().getName() + " method; "
					+ method.getName(), e);
		}
	}
	...
	/** 
	 * Get the value for a field, first the field is accessed directly if not found then 
	...
	public static Field getField(Class clazz, String fieldName) {
		...
		try {
			...
			if (field == null) {
				field = getFieldInternal(clazz, fieldName + "_"); // the way emf escapes fields
			}
		} catch (Exception e) // todo replace with specific exception
		{
			throw new StoreException("Field " + fieldName + " not accessible for class: " + clazz.getName(), e);
		}
		if (field == null) {
			throw new StoreException("Field " + fieldName + " not found in class: " + clazz.getName());
		}
