 ...
 * implementation </copyright> $Id: StoreUtil.java,v 1.25.2.1 2009/06/30 07:29:20 mtaal Exp $
 */
...
public class StoreUtil {
	...
	public static List<EPackage> readEPackages(String[] ecoreFiles) {
		...
		for (String element : ecoreFiles) {
			...
			Resource res = resourceSet.getResource(URI.createFileURI(element),
					true);
			...
			while (it.hasNext()) {
				...
				if (obj instanceof EPackage) {
					...
					final EPackage currentEPackage = EPackage.Registry.INSTANCE
	...
	public static boolean isWildCard(EStructuralFeature feature) {
		...
		return kind != null && kind.compareTo("elementWildcard") == 0
				&& wildcards != null;
		/*
	...
	public static boolean isMixed(EStructuralFeature feature) {
		...
		return kind != null && kind.compareTo("elementWildcard") == 0
				&& name != null && name.compareTo(":mixed") == 0;
		/*
		 * // todo optimise this with a cache final EAnnotation eannotation =
		 * feature
		 * .getEAnnotation("http:///org/eclipse/emf/ecore/util/ExtendedMetaData"
	...
	public static void copyFile(File src, File dst) {
		try {
			...
			throw new TeneoException("Exception while copying from/to "
